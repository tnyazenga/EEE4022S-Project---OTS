...
# And, the distance between every two neighbour blob circle centers is 0.1 centimetres
# In fact, any number can be used to replace 0.1 depending on the grid size used
...
print("Looking for the circle grid...")
while(found<num):  # Here, 10 can be changed to whatever number you like to choose
    ...
    ret, corners = cv2.findCirclesGrid(im_with_keypoints, (7,6), None, flags = cv2.CALIB_CB_ASYMMETRIC_GRID)   # Find the circle grid
    print
    if ret == True:
        print('Grid Found')
        found += 1
        ...
        cv2.imshow('Circle grid found, keep it in frame!', im_with_keypoints)
        ...
        cv2.imshow('Circle grid not found. Press ''q'' to quit.', im_with_keypoints)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
